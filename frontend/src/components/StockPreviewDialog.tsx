          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-border shrink-0">
              ...
              <div className="flex items-center gap-1.5">
                {/* 日期范围快捷 */}
                <button
                  onClick={() => { setViewMode5(true); const e = new Date(); const s2 = new Date(); s2.setDate(s2.getDate() - 6); setDateRange({ start: s2.toISOString().slice(0, 10), end: e.toISOString().slice(0, 10) }); setShowIntraday(false) }}
                  className={`h-6 px-1.5 rounded text-[11px] transition-colors cursor-pointer border border-transparent ${viewMode5 ? 'bg-accent/20 text-accent font-medium border-accent/30' : 'text-muted hover:text-foreground hover:bg-elevated'}`}
                >5日</button>
                {PRESETS.map(p => {
                  const now = new Date()
